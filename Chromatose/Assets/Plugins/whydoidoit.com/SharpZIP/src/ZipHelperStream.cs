...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	internal class ZipHelperStream : Stream
	{
		...
		// ZipHelperStream.WriteLocalHeader is not yet used and needs checking for ZipFile and ZipOuptutStream usage
		...
		public long LocateBlockWithSignature(int signature, long endLocation, int minimumBlockSize, int maximumVariableData)
		{
			...
			// This loop could be optimised for speed.
			do {
				if ( pos < giveUpMarker ) {
					return -1;
				}
			...
			long startOfCentralDirectory, byte[] comment)
		{
			...
			// ZipFile Multi disk handling not done
