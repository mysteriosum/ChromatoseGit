...
public class Drawing
{
	...
	public static void CreateLineTex() {
		if(lineTex != null) {
			return;
		}
		lineTex = new Texture2D(1, 1, TextureFormat.ARGB32, true);
		lineTex.SetPixel(0, 1, Color.gray);
		lineTex.Apply();
		
		lineTex.hideFlags = HideFlags.HideAndDontSave;
	}
	
	public static void DrawLine(Vector2 pointA, float width) {
		CreateLineTex();
	...
	public static void DrawDashLine(Vector2 pointA, float dashWidth, float width) {
		
		float space = dashWidth * 0.3f;
		float total = System.Convert.ToInt32(width / (dashWidth + space));
		
		CreateLineTex();
		
		
		float start = pointA.x;
		for(int i = 0; i < total; i++) {
			GUI.DrawTexture(new Rect(start, pointA.y, dashWidth, 1), lineTex);
			start += dashWidth + space;
		}
    ...
    public static void bezierLine(Vector2 start, Vector2 startTangent, Vector2 end, Vector2 endTangent, Color color, float width, bool antiAlias, int segments)
    {
		...
		segments = System.Convert.ToInt32((Vector2.Distance(start, end) / 10f) * segments);
